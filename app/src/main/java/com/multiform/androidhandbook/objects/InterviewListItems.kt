    ...
    private val androidSdkList = mutableListOf(
        ...
        ListArticleItem("Виды Intent", R.drawable.ic_app_components, false,
            "Intent это объект описания одной операции. С помощью интент можно запросить выполнение действия у приложения. Обычно интент используется в 3 сценариях:\n" +
                    "1. Для запуска операции (Activity): объект Intent описывает операцию, которую требуется запустить, а также содержит все остальные необходимые данные. Объект запускает новую Activity с помощью метода startActivity(). Если после завершения операции от нее требуется получить результат, вызовите метод startActivityForResult(). Ваша операция получит результат в виде отдельного объекта Intent в обратном вызове метода onActivityResult() операции.\n" +
                    "2. Для запуска сервиса (Service): сервис можно запустить для выполнения однократного действия (например, чтобы загрузить файл), передав объект Intent методу startService(). Service является компонентом, который выполняет действия в фоновом режиме без пользовательского интерфейса. Объект Intent описывает службу, которую требуется запустить, а также содержит все остальные необходимые данные. Также можно установить привязку к другму уже запущенному сервису, передав объект Intent методу bindService().\n" +
        ...
        ListArticleItem("ViewGroup", R.drawable.ic_app_components, false,
            "View - это базовый строительный блок для компонентов пользовательского интерфейса. От этого класса наследуются все другие UI компоненты.\n" +
